/**
 * Test TKKT reminders against the real Ectoin / E. coli project on VPS.
 * Temporarily sets leader email + specialist + midterm due = today+30 (VN),
 * runs engine, then restores original project data.
 *
 * Usage: npx tsx src/scripts/testReminderEctoin.ts
 */
...
const TITLE =
  'Khảo sát sản xuất ectoin ở các dòng Escherichia coli đột biến hệ vận chuyển glucose';

const PROJECT_ID = 'bbd2a5ee-3adc-49f6-9be9-bc715925763c';
const SPECIALIST_USERNAME = 'lhthinh@ump.edu.vn';
...
async function main() {
  console.log('=== Test nhắc mail — đề tài Ectoin / E. coli ===');
  ...
  const { rows: specialists } = await pool.query<{ id: string; username: string; display_name: string }>(
    `SELECT id, username, display_name
     FROM users
     WHERE username = $1 OR username = 'lhthinh'
     LIMIT 1`,
    [SPECIALIST_USERNAME],
  );
  const specialist = specialists[0];
  if (!specialist) {
    throw new Error(`Không tìm thấy chuyên viên: ${SPECIALIST_USERNAME}`);
  }
  ...
  console.log('specialist (temp):', specialist.display_name, specialist.username, specialist.id);
